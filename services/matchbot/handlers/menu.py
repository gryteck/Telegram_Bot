...
@dp.message_handler(state=States.menu_answer)
async def menu_answer(message: types.Message):
    ...
    if message.text == "1":
        ...
        await rd.update_state(message.from_user.id, States.delete_confirm)
    else:
        await message.reply(t.invalid_answer)


@dp.message_handler(state=States.cont)
...
@dp.message_handler(state=States.get_photo, content_types=["photo", "video"])
