...
async def choose_interest(message: types.Message, state: FSMContext):
    ...
    except (KeyError, exceptions.BadRequest): await message.answer(t.set_name(), reply_markup=types.ReplyKeyboardRemove())
...
async def name(message: types.Message, state: FSMContext):
    if message.text == "Продолжить":
        return await random_form(message, state, message.from_user.id, db.get_form(message.from_user.id))
    elif len(message.text) not in range(3, 12):
        return await message.answer("Недопустимая длина имени")
    elif t.name_invalid(message.text):
        return await message.reply("Давай что-нибудь посодержательней")
    ...
    await sleep(1)
    try:
        await message.answer('Сколько тебе лет?', reply_markup=kb.custom((await state.get_data())['age']))
    except (KeyError, exceptions.BadRequest):
