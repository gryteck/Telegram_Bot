...
async def reaction_should_be_processed(message: types.Message, f: SUser, l: SUser) -> bool:
    return message.text == "❤️" \
           ...
           and l.visible \
           and not f.banned \
           and (l.id not in await db.get_likes(message.from_user.id))
...
async def reaction_processing(message: types.Message, l: SUser):
    ...
    if message.from_user.id not in l.liked:
        ...
        liked_buffer = await db.filter_liked(l.liked+[message.from_user.id])

        if len(liked_buffer) <= settings.LIKED_BUFFER:
            l = await db.update_user(l.id, liked=liked_buffer)
